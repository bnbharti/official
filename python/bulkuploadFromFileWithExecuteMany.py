...
def bulk_update_from_csv(file_path):
    ...
    try:
        ...
        # Read the CSV file and prepare data for bulk insert
        with open(file_path, mode='r') as file:
            ...
            if sum(1 for row in reader) > 0:
                ...
                data_to_insert = []
                for row in reader:
                    data_to_insert.append((row[0], row[1], row[2]))  # Append each row as a tuple
                ...
                records_loaded = len(data_to_insert)  # Number of records loaded
